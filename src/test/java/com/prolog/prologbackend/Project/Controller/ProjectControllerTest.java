...
//         given
    ...
    void 프로젝트_리스트_반환() throws Exception {
         ...
         */
        // given
        Long invalidUserId = null; // 유효하지 않은 유저 ID (null)

        // when & then
        mvc.perform(get(BASE_URL + "/list")
                        .with(csrf())
                        .param("userId", invalidUserId != null ? invalidUserId.toString() : ""))
                .andExpect(status().isBadRequest())
    ...
    void 프로젝트_생성() throws Exception {
        ...
        mvc.perform(post(BASE_URL + "/insert")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(requestProjectDetailDTO)))
                .andExpect(status().isBadRequest());
    ...
    void 프로젝트_삭제() throws Exception{
        ...
        mvc.perform(put(BASE_URL + "/delete")
                        .with(csrf())
                        .param("projectId",InvalidProjectId != null ? InvalidProjectId.toString() : ""))
                .andExpect(status().isBadRequest());
